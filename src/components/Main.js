...
function Main() {
  const [img, setImg] = useState([{link: "https://source.unsplash.com/random/800x600", title: "https://source.unsplash.com/random/800x600"}, {link: "https://source.unsplash.com/random/800x600", title: "https://source.unsplash.com/random/800x600"}]);
  ...
  async function fetchNext() {
	setLoading(true);
    let res = await axios.get("https://nutornot.el.r.appspot.com/pvp");
    console.log(res.data);
    setImg(res.data);
	setLoading(false);
  }

  async function Nut(e) {
    setLoading(true);
    if(e.target.name === "one") {
	await axios.get(`https://nutornot.el.r.appspot.com/image/add/${img[0].title}`);
    }
    else {
	await axios.get(`https://nutornot.el.r.appspot.com/image/add/${img[1].title}`);
    }
    fetchNext();
  }
      ...
      {loading ? (
        ...
        <>
          <div className="mb-4 mt-6 md:w-96 w-88 h-2/3 flex items-center justify-center overflow-hidden rounded-lg shadow-lg">
            <img src={img[0].link} alt="logo" className="w-full h-full object-cover" onClick={Nut} name="one" />
            <img src={img[1].link} alt="logo" className="w-full h-full object-cover" onClick={Nut} name="two" />
          </div>
